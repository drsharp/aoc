...
@beams = Array.new(first_row.length, 0)
@beams[first_row.index("S")] = 1 # first has "S". First beam
...
rows.each_with_index do |row, r_idx|
  ...
  carats.each do |carat_column|
    if @beams[carat_column] > 0 # has at least 1 beam
      @beams[carat_column-1] = @beams[carat_column-1] + @beams[carat_column] # add all beams at column to prev column
      @beams[carat_column+1] = @beams[carat_column+1] + @beams[carat_column] # add all beams at column to next column
      @beams[carat_column] = 0 # split so none here anymore
    end
  end
end

puts "Solution for Day 7, Part 2: #{@beams.sum}"
