...
class Lights
  def initialize(rule, combo)
    @rule = rule
    @bulbs = Array.new(rule.length, false) # all are off
    @combo = combo
  end
  ...
  def toggle!
    @combo.each do |button|
      button.split(",").each do |position|
        @bulbs[position.to_i] = !@bulbs[position.to_i]
      end
    end
  end
...
puts "Solution for Day 10, Part 1: #{@solution}"
